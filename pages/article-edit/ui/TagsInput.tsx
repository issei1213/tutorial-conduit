...
export function TagsInput({ name, defaultValue }: { name: string; defaultValue?: Array<string> }) {
  ...
  return (
    <>
      <input
        ...
        onChange={(e) => setTagListState(e.target.value.split(",").filter(Boolean))}
      />
        ...
        {tagListState.map((tag) => (
          <span className="tag-default tag-pill" key={tag}>
            <i
              ...
              role="button"
              tabIndex={0}
              onKeyDown={(e) => [" ", "Enter"].includes(e.key) && removeTag(tag)}
